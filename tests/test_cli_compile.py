...
def test_command_line_overrides_pip_conf(pip_with_index_conf, runner):
    ...
    with open("requirements.in", "w"):
        pass
    out = runner.invoke(compile, ["-v", "-i", "http://override.com"])
...
def test_command_line_setuptools_read(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-emit-find-links"])
...
def test_command_line_setuptools_output_file(
    ...
    out = runner.invoke(compile, options)
    assert out.exit_code == 0
    assert out.stdout == ""
...
def test_find_links_option(runner):
    ...
    out = runner.invoke(compile, ["-v", "-f", "./libs1", "-f", "./libs2"])
...
def test_all_no_emit_options(runner, options):
    with open("requirements.in", "w"):
        pass
    out = runner.invoke(compile, ["--no-header"] + options)
    assert out.exit_code == 0
    assert out.stdout == ""
...
def test_emit_index_url_option(runner, option, expected_output):
    ...
    out = runner.invoke(
        compile, ["--no-header", "--index-url", "https://index-url", option]
    )
...
def test_realistic_complex_sub_dependencies(runner):
    ...
    out = runner.invoke(compile, ["-v", "-n", "--rebuild", "-f", wheels_dir])
...
def test_run_as_module_compile():
    ...
    status, out, err = invoke([sys.executable, "-m", "piptools", "compile", "--help"])

    # TOOD: ASSERT THE FULL output?
    # Should have run pip-compile successfully.
    assert status == 0
    assert out.startswith(b"usage:")
    assert b"Compiles requirements.txt from requirements.in" in out
    assert err == b""
...
def test_editable_package_vcs(runner):
    ...
    out = runner.invoke(compile, ["-n", "--rebuild"])
    assert out.exit_code == 0
    assert out.stdout == ""
...
def test_locally_available_editable_package_is_not_archived_in_cache_dir(
    ...
    out = runner.invoke(compile, ["-n", "--rebuild", "--cache-dir", str(cache_dir)])
...
def test_input_file_without_extension(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["requirements"])
...
def test_upgrade_packages_option(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-annotate", "-P", "small-fake-b"])
...
def test_upgrade_packages_option_irrelevant(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-annotate", "--upgrade-package", "small-fake-b"])
...
def test_upgrade_packages_version_option(
    ...
    out = runner.invoke(
        compile, ["--no-annotate", "--upgrade-package", upgraded_package]
    )
...
def test_upgrade_packages_version_option_no_existing_file(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["-P", "small-fake-b==0.2"])
...
def test_upgrade_packages_version_option_and_upgrade(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--upgrade", "-P", "small-fake-b==0.1"])
...
def test_quiet_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(compile, ["--quiet", "-n", "requirements"])
...
def test_dry_run_noisy_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(compile, ["--dry-run", "requirements"])
...
def test_dry_run_quiet_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(compile, ["--dry-run", "--quiet", "requirements"])
...
def test_generate_hashes_with_editable(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-annotate", "--generate-hashes"])
    expected = (
        "-e {}\n"
        "small-fake-a==0.1 \\\n"
...
def test_generate_hashes_verbose(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--generate-hashes", "-v"])
...
def test_generate_hashes_with_annotations(runner):
    ...
    out = runner.invoke(compile, ["--generate-hashes"])
    assert out.stderr == dedent(
        """\
        #
        # This file is autogenerated by pip-compile
...
def test_filter_pip_markers(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["-n", "requirements"])
...
def test_no_candidates_pre(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["-n", "requirements", "--pre"])
...
def test_multiple_input_files_without_output_file(runner):
    ...
    out = runner.invoke(compile, ["src_file1.in", "src_file2.in"])
...
def test_annotate_option(pip_conf, runner, option, expected):
    ...
    out = runner.invoke(compile, [option, "-n", "--no-emit-find-links"])
...
def test_allow_unsafe_option(pip_conf, monkeypatch, runner, option, expected):
    ...
    out = runner.invoke(compile, ["--no-annotate", option] if option else [])
...
def test_cert_option(parse_requirements, runner, option, attr, expected):
    ...
    out = runner.invoke(compile, [option, expected])
...
def test_build_isolation_option(parse_requirements, runner, option, expected):
    ...
    out = runner.invoke(compile, [option])
...
def test_forwarded_args(PyPIRepository, runner):
    ...
    out = runner.invoke(compile, cli_args + ["--pip-args=" + " ".join(pip_args)])
...
def test_pre_option(pip_conf, runner, cli_option, infile_option, expected_package):
    ...
    out = runner.invoke(
        compile, ["--no-annotate", "-n"] + (["-p"] if cli_option else [])
    )
...
def test_dry_run_option(pip_conf, runner, add_options):
    ...
    out = runner.invoke(compile, ["--no-annotate", "--dry-run"] + add_options)
...
def test_empty_input_file_no_header(runner, empty_input_pkg, prior_output_pkg):
    ...
    out = runner.invoke(compile, ["--no-header", "requirements.in"])
...
def test_upgrade_package_doesnt_remove_annotation(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["-P", "small-fake-a", "--no-emit-find-links"])
...
def test_unreachable_index_urls(runner, cli_options, expected_message):
    ...
    out = runner.invoke(compile, cli_options)
...
def test_upgrade_packages_option_subdependency(
    ...
    out = runner.invoke(
        compile, ["--no-annotate", "--dry-run", "--upgrade-package", upgraded_package]
    )
...
def test_remove_outdated_options(runner, input_opts, output_opts):
    ...
    out = runner.invoke(compile, ["--no-header"])
...
def test_sub_dependencies_with_constraints(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-annotate"])
...
def test_preserve_compiled_prerelease_version(pip_conf, runner):
    ...
    out = runner.invoke(compile, ["--no-annotate", "--no-header"])
...
def test_duplicate_reqs_combined(
    ...
    out = runner.invoke(compile, ["--find-links", str(dists_dir)])
